...
export const ImportAttendanceDialog = ({ onAttendanceImported }: ImportAttendanceDialogProps) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const required = ["student_email", "class_subject", "date", "status"];
        ...
        const records: ParsedAttendance[] = rows
          .filter((r) => r.student_email && r.class_subject && r.date && r.status)
          .map((r) => ({
            id: r.id || undefined,
        ...
        if (invalid.length) {
          setParseError(`Invalid status values found. Must be: present or absent. Found: ${[...new Set(invalid.map((r) => r.status))].join(", ")}`);
          setParsedData([]);
          return;
        }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (const record of parsedData) {
        if (!isValidEmail(record.student_email)) {
          errors.push(`${record.student_email}: invalid email format`);
          failed++;
        }
      }
      const validRecords = parsedData.filter((r) => isValidEmail(r.student_email));

      const emails = [...new Set(validRecords.map((r) => r.student_email))];
      ...
      for (const record of validRecords) {
        try {
          const userId = emailToUserId[record.student_email];
          if (!userId) { errors.push(`Row ${record.student_email}/${record.date}: student not found`); failed++; continue; }
          const studentId = userIdToStudentId[userId];
          if (!studentId) { errors.push(`Row ${record.student_email}/${record.date}: student record not found`); failed++; continue; }
          const classId = subjectToClassId[record.class_subject];
          if (!classId) { errors.push(`Row ${record.student_email}/${record.date}: class '${record.class_subject}' not found`); failed++; continue; }

          const parsedDate = parseDateValue(record.date);
          if (!parsedDate) { errors.push(`Row ${record.student_email}: invalid date "${record.date}"`); failed++; continue; }

          let existing: { id: string } | null = null;
          if (record.id) {
            const { data } = await supabase.from("attendance").select("id").eq("id", record.id).maybeSingle();
            existing = data;
          }
          if (!existing) {
            const { data } = await supabase.from("attendance").select("id").eq("student_id", studentId).eq("class_id", classId).eq("date", parsedDate).maybeSingle();
            existing = data;
          }

          if (existing) {
            if (!upsertMode) {
              errors.push(`Row ${record.student_email}/${record.date}: duplicate (enable upsert to update)`);
              failed++;
              continue;
            }
            ...
            if (error) { errors.push(`Row ${record.student_email}/${record.date}: ${error.message}`); failed++; }
            else updated++;
          } else {
          ...
          errors.push(`Row ${record.student_email}/${record.date}: ${err.message}`); failed++;
        }
      }
    ...
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetState(); }}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            CSV columns: <code>student_email, class_subject, date, status</code>.
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <Checkbox id="upsert-attendance" checked={upsertMode} onCheckedChange={(v) => setUpsertMode(!!v)} />
            <Label htmlFor="upsert-attendance" className="text-sm cursor-pointer">
              <RefreshCw className="h-3 w-3 inline mr-1" />
              Update existing records (if same student + class + date found, update instead of failing)
            </Label>
          </div>
          {parseError && (
