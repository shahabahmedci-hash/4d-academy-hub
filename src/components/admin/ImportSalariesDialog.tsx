...
interface ImportSalariesDialogProps {
  onSalariesImported: () => void;
}

export const ImportSalariesDialog = ({ onSalariesImported }: ImportSalariesDialogProps) => {
  const { toast } = useToast();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [open, setOpen] = useState(false);
  const [parsedData, setParsedData] = useState<ParsedSalary[]>([]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const required = ["teacher_email", "amount", "month"];
        ...
        const records: ParsedSalary[] = rows
          .filter((r) => r.teacher_email && r.amount && r.month)
          .map((r) => ({
            id: r.id || undefined,
        ...
        if (!records.length) {
          setParseError("No valid rows found. Ensure 'teacher_email', 'amount', and 'month' are filled.");
          setParsedData([]);
          return;
        }
    ...
    try {
      ...
      for (const record of validRecords) {
        try {
          ...
          if (!userId) { errors.push(`${record.teacher_email}: teacher not found`); failed++; continue; }
          const teacherId = userIdToTeacherId[userId];
          if (!teacherId) { errors.push(`${record.teacher_email}: teacher record not found`); failed++; continue; }

          const amount = parseNumericValue(record.amount);
          if (amount <= 0) { errors.push(`${record.teacher_email}: invalid amount`); failed++; continue; }

          const status = record.status && VALID_STATUSES.includes(record.status.toLowerCase()) ? record.status.toLowerCase() : "pending";

          // Normalize month: handle YYYY-MM, DD/MM/YYYY, etc.
          let monthDate: string | null;
          if (/^\d{4}-\d{1,2}$/.test(record.month.trim())) {
            monthDate = `${record.month.trim()}-01`;
          } else {
            monthDate = parseDateValue(record.month);
          }
          if (!monthDate) { errors.push(`${record.teacher_email}: invalid month "${record.month}"`); failed++; continue; }

          let existing: { id: string } | null = null;
          if (record.id) {
            const { data } = await supabase.from("teacher_salaries").select("id").eq("id", record.id).maybeSingle();
            existing = data;
          }
          if (!existing) {
            const { data } = await supabase.from("teacher_salaries").select("id").eq("teacher_id", teacherId).eq("month", monthDate).maybeSingle();
            existing = data;
          }

          if (existing) {
            if (!upsertMode) {
              errors.push(`${record.teacher_email}/${record.month}: duplicate (enable upsert to update)`);
              failed++;
              continue;
            }
            ...
            if (error) { errors.push(`${record.teacher_email}/${record.month}: ${error.message}`); failed++; }
            else updated++;
          } else {
          ...
          errors.push(`${record.teacher_email}: ${err.message}`); failed++;
        }
      }
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetState(); }}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Import Salaries from CSV</DialogTitle>
          <DialogDescription>
            CSV columns: <code>teacher_email, amount, month, status, payment_method, notes</code>.
            Required: teacher_email, amount, month (YYYY-MM or DD/MM/YYYY). Status: pending or paid.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <Checkbox id="upsert-salaries" checked={upsertMode} onCheckedChange={(v) => setUpsertMode(!!v)} />
            <Label htmlFor="upsert-salaries" className="text-sm cursor-pointer">
              <RefreshCw className="h-3 w-3 inline mr-1" />
              Update existing records (if same teacher + month found, update instead of failing)
            </Label>
          </div>
          {parseError && (
