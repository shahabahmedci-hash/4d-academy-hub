...
export async function generateSalaryReceipt(data: SalaryReceiptData) {
  const receiptNumber = `SAL-${data.salaryId.substring(0, 8).toUpperCase()}`;
  const doc = new jsPDF({ unit: "mm", format: "a5" });
  const w = doc.internal.pageSize.getWidth();
  const monthLabel = new Date(data.month).toLocaleDateString("en-IN", { month: "long", year: "numeric" });
  ...
  doc.text("Salary Receipt", w / 2, 26, { align: "center" });
  ...
  const filename = `SalaryReceipt-${receiptNumber}.pdf`;
